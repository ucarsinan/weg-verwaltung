...
--
-- Legal grounding:
--   § 16 Abs. 2 WEG makes MEA the default distribution and lets the owners
--   resolve a different key per individual cost item or per cost type. That is
--   exactly the 0056 shape: the key hangs off the position
--   (wirtschaftsplan_position.verteilungsschluessel_version_id), not off the
--   WEG. § 28 WEG makes the resulting Vorschuesse (Hausgeld) the thing that is
--   resolved — which is what a Sollstellung row materializes.
--
-- Risk posture:
--   - No change to activate_wirtschaftsplan(), archive_wirtschaftsplan(),
--     create_nachtragsplan(), RLS, or agent guards.
--   - No change to sollstellung's insert-only lock-down (0039/0040/0042/0047)
--     or its conflict target; only the source query inside the existing
--     internal generator changes.
--   - Plans with zero wirtschaftsplan_position rows produce byte-identical
--     output to the pre-0060 generator: that branch is a verbatim copy of the
--     0047 body, including the /12.0 divisor for partial-year plans (the
--     monthly rate stays an annual budget / 12; a plan starting in July simply
--     posts six rows).
...
--     basiswert is unique per (tenant_id, version_id, unit_id, gueltig_ab) and
--     has no column saying which part of a mixed rule a row belongs to, so a
--     70/30 rule cannot store a unit's consumption AND its area. Resolving
--     that needs a schema/product decision, not a silent assumption in a
--     money-calculation path. Note this blocks HeizKV-style heating costs,
--     which by law MUST be split 30-50 % base cost by area and 50-70 % by
--     metered consumption — tracked as the next finance slice.
...
--     partial total.
--
-- Deliberate deviation from the pre-0060 MEA arithmetic (positions path only):
--   The 'mea' branch NORMALIZES each unit's MEA fraction by the sum of all MEA
--   fractions in the WEG. Nothing in the schema forces a WEG's MEA to add up
--   to 1 (0003 only checks mea_zaehler > 0 and mea_nenner > 0). Using the raw
--   fractions would leave part of a position's jahresbetrag unallocated
--   whenever they do not — the same class of silent mis-allocation this
--   migration fails closed on for missing basis values. § 16 Abs. 2 WEG
--   apportions costs "im Verhaeltnis der Miteigentumsanteile", i.e. by the
--   ratio between owners, and the budget has to be funded in full. Normalizing
--   makes the helper's "shares sum to 1" contract actually true and keeps mea
--   consistent with flaeche/verbrauch/manuell, which already normalize. The
--   legacy no-positions branch is intentionally NOT changed.
--
--   Residual rounding: monthly instalments are round(total / 12, 2) per unit,
--   so twelve instalments need not re-add to the position total to the cent.
--   That is inherent to monthly Vorschuesse and matches the pre-0060 path.
--
...
begin
  ...
  if v_typ = 'mea' then
    -- Normalize by the WEG's MEA total; see the header note on why the raw
    -- fraction is not used here.
...
comment on function private._verteilungsschluessel_version_unit_shares(uuid, uuid, uuid, date) is
  'Internal generator helper. Returns fractional per-unit shares summing to 1 for a single allocation key version. Normalizes mea by the WEG MEA total. Fails closed for typ=gemischt (0A000) and for missing basis-value coverage (23514) instead of silently mis-allocating.';
...
begin
  ...
  -- Positions exist: allocate each position's jahresbetrag via its allocation
  -- key, sum per unit across all positions, then split into equal monthly
  -- instalments the same way the MEA path always has. The basis-value
  -- reference date is 1 January of the plan year: a Wirtschaftsplan budgets a
  -- calendar year even when it only becomes effective mid-year.
  ...
  from (
    ...
    cross join pg_catalog.generate_series(v_start_month, 12) as months(monat)
    order by totals.unit_id, months.monat
  ) as generated
  on conflict (tenant_id, wirtschaftsplan_id, unit_id, monat)
    where buchungstyp = 'initial'
  do nothing;
