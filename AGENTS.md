...
just test-audit-db # nur die Audit-Vertraege (0002, 0046, 0055, 0058, 0059, 0068, 0069, 0071); 0069/0071 decken auch 0072 ab
...
- pgTAP-Verträge `0050`, `0052`, `0054` sind rot und laufen deshalb in keinem Rezept und in keinem CI-Gate (Stand 2026-09-19, lokal gegen eine ephemere DB geprüft): `0050_audit_console_read_api` 9 von 54 Assertions rot; `0052_vorgangszentrale_foundation` bricht nach 14 von 28 ab; `0054_agent_suggestion_vorgang_anchor` stirbt an `permission denied for schema auth` in `audit_writer.tg_emit_vorgang_audit_event` beim Aufruf von `auth.uid()`. Letzteres sieht nach einer Lücke im lokalen Bootstrap aus (`infra/supabase/ci/audit_regression_bootstrap.sql` normalisiert Grants für `vault`/`pgsodium`, aber nicht `usage on schema auth` für `audit_writer`) und nicht zwingend nach einem Produktfehler — in der gehosteten Umgebung hat die Rolle vermutlich mehr Rechte als nach `supabase db reset`. Nicht verifiziert. Vor der Aufnahme in `test-db-all` erst grün machen, nicht die Tests abschwächen.
