...
| `P1` (Security-relevant) | `rls.spec.ts:43` (`rls-wp-select-isolated`) und `:78` (`rls-sollstellung-select-isolated`) pruefen nichts Konkretes — `ok()\|\|404`-Tautologie plus mathematisch immer-wahres `toBeGreaterThanOrEqual(0)` | Diese Tests sollen laut `AGENTS.md` „Tenant-Isolation ist nicht verhandelbar" beweisen, tun es aber nicht — falsches Sicherheitsgefuehl bei einem sicherheitskritischen Invariant |
