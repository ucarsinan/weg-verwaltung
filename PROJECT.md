...
Dieser Status konsolidiert den lokalen Stand. Der reale Codezustand gilt vor aelteren Statusnotizen: lokal existieren Supabase-Migrationen bis `0067`, eine Next.js-16-Web-App, ein FastAPI/LangGraph-Agent, 19 E2E-Specs und Audit-/Finance-/Meeting-/Vorgangs-/Advisor-Hardening-Artefakte.

**Lokal belegt:** Migrationen `0001-0067` liegen unter `infra/supabase/migrations`. `0045`/`0046` bilden den Audit-Forward-Repair ab, `0047`/`0048` das Finance-Lifecycle-Hardening, `0049` Meeting/Resolution-Hardening, `0050` die Audit-Console-Read-API, `0051` den Actor-Guard-DELETE-Fix, `0052` die Vorgangszentrale-Foundation, `0053` Settings-Audit-Trigger, `0054` den Agent-Suggestion-Vorgangsanker, `0055` Advisor-Grant-/RLS-InitPlan-Hardening, `0056` die Finance-Allocation-Foundation und `0057` die Self-Managed-SaaS-Foundation. Seit `0060` ist die Finanzkette gebaut: positionsgenaue Verteilung (`0060`), Zahlungen und offene Posten (`0061`), Ausgaben und Erhaltungsruecklage (`0062`), Jahresabrechnung nach § 28 Abs. 2 WEG (`0063`), NULL-sichere Writer-Guards (`0064`), Vermoegensbericht nach § 28 Abs. 4 WEG (`0065`), loeschbarer Abrechnungsentwurf (`0066`) und gemischte Verteilungsschluessel mit erzwungenem HeizkostenV-Korridor (`0067`). **Damit ist die Pflichtkette aus § 28 WEG im Datenmodell vollstaendig.** Ebenfalls belegt: eine TOM-Liste nach Art. 32 DSGVO mit Nachweis je Massnahme (`docs/09-tom-art32.md`).

**Nicht in diesem Audit verifiziert:** Cloud-Migrationsstand (zuletzt ausgerollt am 2026-09-20, seither kein `supabase migration list --linked`), Zahlen des letzten vollstaendigen Cloud-E2E-Laufs, produktives Web-/Agent-Hosting, Backup/Restore.

**Ampel:** Gruen fuer den lokalen Codestand — Worktree sauber, `main` aktuell, alle CI-Gates auf PR #22 gruen. Gelb fuer Release-Readiness: es fehlen Backup/Wiederherstellung, eine katalogweite RLS-Zusicherung, AVV und Art.-30-Verzeichnis sowie ein belegter Cloud-Abgleich. Rot bleibt jeder Claim Richtung „produktionsreif" oder „DSGVO-konform", solange diese Punkte offen sind.
...
- **Finanzen Module (`modules/finanzen`)**: Exposes database structures for `wirtschaftsplan`, `sollstellung`, versioned allocation keys (including composed `gemischt` keys under the HeizkostenV corridor), allocation basis values, Wirtschaftsplan positions, payments and open items, expenses and the Erhaltungsruecklage, Jahresabrechnung, and the Vermoegensbericht. Option B is binding: `sollstellung` rows are historical, materialized unit-month targets and are never recalculated, overwritten, or deleted because a plan, unit, or MEA value changes later.
